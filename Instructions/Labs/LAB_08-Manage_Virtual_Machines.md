...
1. Click **Next: Disks >** on the **Disks** tab, review the available settings without modifying any of them.
...
1. Click **OK** and, back on the **Networking** tab of the **Create a virtual machine** blade, check or ammend the following settings (leave others with their default values):
    ...
    | Public inbound Ports | **None** |
    | Accelerated networking | **Unticked**
...
1. Once the deployment starts. On the **CreateVM-** deployment blade, click **Template**.
...
1. Click **Review + Create**, once validation has passed, click **Create**.
...
1. On the **New container** blade, specify the following settings (leave others with their default values) and click **Create**:
    ...
    | Setting | Value |
    | --- | --- |
    | Name | **scripts** (must be lower case) |
...
1. Back on the **Install extension** blade, click **Review + create** and when validation has passed, click **Create**.
...
1. On the **Edit template** blade, in the section displaying the content of the template.  **Place the cursor at the end of line 19** and press the **Enter** key to create a new blank line **20**. insert the following code at line **20** (directly underneath the `"resources": [` line):
...
1. Click **Save** and, back on the **Custom template** blade, click **Review + Create** and when validation has passed, click **Create**
...
1. On the **Run Command Script** blade, type the following and click **Run** to access the web site hosted on **az104-08-vm0**:
   ...
   ```
    ...
    Look for a return value of **OK** for the **StatusDescription**.  This can take a couple of minutes to complete.
...
1. On the **az104-08-vm0** virtual machine blade, click **Size** and in the list select the virtual machine size **Standard DS1_v2** and click **Resize**.  You do not need to wait for it to complete, you can proceed to the next task.
...
1. On the **Run Command Script** blade, copy the following code and paste it into the PowerShell script window, and click **Run** to create a drive Z: consisting of the two newly attached disks with the simple layout and fixed provisioning:
...
1. On the **Edit template** blade, in the section displaying the content of the template, ***around the area of line 50***, find the following line ***`"dataDisks": [ ],`*** and replace it with the following code :
   ...
   ```
    ...
    >**Note**: If you are using a tool that pastes the code in line by line intellisense may add extra brackets causing validation errors. You may want to paste the code into notepad first and then paste it into line 51.
...
1. Click **Save** and, back on the **Custom deployment** blade, click **Review + Create** and once validation has passed, click **Create**.
...
1. On the **Edit network interface** blade, under the **Public inbound ports** section, select the **Allow selected ports** radio button and in the **select inbound ports** drop down list, select **HTTP(80)**.

1. In the **Public IP address** section, click **Enabled** and click **OK**.

1. Back on the **Networking** tab of the **Create a virtual machine scale set** blade, under the **Load balancing options** section, ensure that the **Azure load balancer** radio button entry is selected. 

1. Under the **Select a load balancer** section, click **Create a load balancer** specify the following **settings** (leave others with their default values) and then click **Create**:
...
1. On the **Management** tab of the **Create a virtual machine scale set** blade, specify the following settings (leave others with their default values):
    ...
    | --- | --- |
    | Boot diagnostics | **Enable with custom storage account** |
    | Diagnostics storage account | In the **Drop down list**: Select the storage account you created in Task 1 |
...
1.  On the **Advanced** tab of the **Create a virtual machine scale set** blade, review the default settings without making any changes and click **Review + create**.
...
1. On the **Install an Extention** blade, search for and select, **Custom Script Extension** and then click **Next**.

1. From the **Install extension** blade, click **Browse**.

1. In the **Storage accounts** blade, select the storage account that was previously selected for the ScaleSet

1. Select the **scripts** container and then select the **az104-08-install_IIS.ps1** script, and then click **Select**.
...
1. On the **az10408vmss0-lb** blade, in the **settings** section: click **Frontend IP configuration** and note the value of the **Public IP address** assigned to the frontend of the load balancer, open an new browser tab, and navigate to that IP address.
...
1. In the **az10408vmss0** blade, in the **Availability + Scale** section, click **Size**.
...
1. Return to the **az10408vmss0 - Instances** blade, and in the **Availability + Scale** section, click **Scaling**.
...
1. Click **Add** and, back on the **az10408vmss0 - Scaling** blade, set the following **instance limits** settings:
...
1. From the Cloud Shell pane, run the following to identify the public IP address of the load balancer in front of the Azure virtual machine scale set **az10408vmss0**.
   ...
   $pip = (Get-AzPublicIpAddress -ResourceGroupName $rgName).IpAddress
   ```
...
1. Click **Save**
    ...
    >**Note:**  Wait for the Disk provisioning to complete before moving on to the next step.
...
1. Wait for the script to finish executing, then close the Cloud Shell pane.
...
1. In the Azure portal, open a **PowerShell** session within the **Cloud Shell** pane.

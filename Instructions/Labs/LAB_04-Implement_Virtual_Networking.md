...
1. Back on the **ipconfig1** blade, **save** the changes.
    ...
    >**Note**: Make sure to wait for the save operation to complete before you proceed to the next step.
...
1. Click **+Add** to add an inbound rule with the following settings (leave others with their default values): Click **Add**
...
1. Associate the **az104-04-nsg01** network security group with the **az104-04-nic0** network interface and click **Ok**. Repeat for the **az104-04-nic1** network interface.
...
2. Create a DNS zone with the following settings (leave others with their default values):
...
6. Add a record set with the following settings (leave others with their default values):
...
9. Add a record set with the following settings (leave others with their default values):
...
13. From the Cloud Shell pane, run the following to test external name resolution of the **az104-04-vm0** DNS record set in the the newly created DNS zone (**Replace the placeholder** `[Name server 1]` with the name of **Name server 1** you noted earlier in this task):

   ```powershell
   nslookup az104-04-vm0.Adatum.com [Name server 1]
   ```

14. Verify that the output of the command includes the public IP address of **az104-04-vm0**.

15. From the Cloud Shell pane, run the following to test external name resolution of the **az104-04-vm1** DNS record set in the the newly created DNS zone (**Replace the placeholder** `[Name server 1]` with the name of **Name server 1** you noted earlier in this task):

   ```powershell
   nslookup az104-04-vm1.Adatum.com [Name server 1]
   ```

...
1. In the Azure portal, search for and select **App services**, and, on the **App Services** blade, click **+ Create**, then select **+ Web App**.
...
1. Click **Review + create**. On the **Review + create** tab click **Create**.
...
1. Click **Add**. Wait for the **staging** slot to be created then click **Close**.
...
1. On the **Deployment Center** blade, select the **Local Git/FTPS credentials** tab, in the **User Scope** section, specify the following settings.
...
1. If prompted to authenticate, type the `[deployment_user_name]` and the corresponding password (which you set in the previous task).
    ...
    >**Note:** When typing the password, the cursor will remain static and flashing on the screen.  This is to be expected.

1. On completion of deployment, close the Cloud Shell pane.
...
2. On the **Swap** blade, review the default settings and click **Swap**. Wait for the swap to complete and click **Close**.

3. Click **Overview** on the production slot blade of the web app and then click the **URL** link to display the web site home page in a new browser tab.

4. Verify the default web page has been replaced with the **Hello World!** page. Close the new browser tab.
...
1. In the **Scaling section**, under **Scale out method**, select the **Rules based** radio button.

1. Select the **Manage rules based scaling** link

On the **Autoscale setting** blade.

1. Click **Custom autoscale**.

    >**Note**: You also have the option of scaling the web app manually.

1. For **Scale mode**, select **Scale based on a metric** and click **Add a rule**
...
1. On the **Scale rule** blade, specify the following settings (leave others with their default values):
    ...
    | Duration (in minutes) | **1** (Ignore the warning) |
...
2. Click **Add** and, back on the App Service plan scaling blade, specify the following settings (leave others with their default values):
...
6. From the Cloud Shell pane, run the following to identify the URL of the Azure web app.
...
7. From the Cloud Shell pane, run the following to start and infinite loop that sends the HTTP requests to the web app:

   ```powershell
   while ($true) { Invoke-WebRequest -Uri $webapp.DefaultHostName }
   ```

8. Minimize the Cloud Shell pane (but do not close it) and, on the web app blade, in the Settings section, click **Scale out (App Service plan)**.

9. Select the **Run history** tab, and check the **Observed resource instance count**.

10. Monitor the utilization and the number of instances for a few minutes. 

    >**Note**: You may need to **Refresh** the page.

11. Once you notice that the number of instances has increased to 2, reopen the Cloud Shell pane and terminate the script by pressing **Ctrl+C**.
